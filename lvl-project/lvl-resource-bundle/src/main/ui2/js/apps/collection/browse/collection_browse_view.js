...
define([ 'app', 'tpl!apps/collection/browse/templates/collection_browse', 'tpl!apps/collection/browse/templates/toolbar_browse',
		'tpl!apps/collection/browse/templates/search_term', 'tpl!apps/collection/browse/templates/add_search_term', 'apps/config/marionette/styles/style',
		'entities/sequence', 'pace', 'common/country_names', 'backbone.oauth2', 'backgrid', 'backgrid-paginator', 'backgrid-select-all', 'backgrid-filter' ],
		function(Lvl, BrowseTpl, ToolbarTpl, SearchTermTpl, AddSearchTermTpl, Style, SequenceModel, pace, mapCn) {
							...
							cell : Backgrid.Cell.extend({
								render : function() {
									...
									if (formattedValue && typeof formattedValue === 'string') {
										...
										if (countryName) {
											this.$el.append('<a href="/#collection/map/country/' + code2.toLowerCase()
													+ '"><img src="img/blank.gif" class="flag flag-' + code2.toLowerCase() + '" alt="' + countryName
													+ '" /><span class="hidden-xs"> ' + countryName + '</span></a>');
										}
									}
									this.delegateEvents();
									return this;
