...
/**
 * Leshmania samples resource.
 * @author Erik Torres <ertorser@upv.es>
 */
@Path("/samples/leishmania")
public class LeishmaniaSampleResource {

	public static final String RESOURCE_NAME = ConfigurationManager.LVL_NAME + " Sample (leishmania) Resource";

	protected final static Logger LOGGER = getLogger(LeishmaniaSampleResource.class);
	...
	public Identifiers getIdentifiers(final @QueryParam("q") @DefaultValue("") String q,			
		...
		final ImmutableMap<String, String> filter = parseQuery(q);
		final Sorting sorting = parseSorting(sort, order);		
		final List<LeishmaniaSample> samples = LEISHMANIA_SAMPLE_DAO.list(0, Integer.MAX_VALUE, filter, sorting, 
				ImmutableMap.of("leishmaniaSample.collectionId", true, "leishmaniaSample.catalogNumber", true), null);
		// process and return to the caller
		final Set<String> ids = ofNullable(samples).orElse(Collections.<LeishmaniaSample>emptyList()).stream().map(s -> {
	...
	@Produces(APPLICATION_JSON)
	public LeishmaniaSample getSample(final @PathParam("id") String id, final @Context UriInfo uriInfo,
	...
	public Response createSample(final LeishmaniaSample sample, final @Context UriInfo uriInfo,
			final @Context HttpServletRequest request, final @Context HttpHeaders headers) {
		OAuth2SecurityManager.login(request, null, headers, RESOURCE_NAME).requiresPermissions("samples:leishmania:*:*:create");
	...
	@Path("{id: " + US_ASCII_PRINTABLE_PATTERN + "}")
	@Consumes(APPLICATION_JSON)
	public void updateSample(final @PathParam("id") String id, final LeishmaniaSample update,
			final @Context HttpServletRequest request, final @Context HttpHeaders headers) {
		if (isBlank(id)) {
			throw new WebApplicationException("Missing required parameters", Response.Status.BAD_REQUEST);
		}
		...
		// update
		LEISHMANIA_SAMPLE_DAO.update(update);			
	}
	...
	public void deleteSample(final @PathParam("id") String id, final @Context HttpServletRequest request, 
		...
		// delete
		LEISHMANIA_SAMPLE_DAO.delete(sampleKey);
	}
	...
	public FeatureCollection findNearbySamples(final @PathParam("longitude") double longitude, 
			...
			final @Context HttpHeaders headers) {
		OAuth2SecurityManager.login(request, null, headers, RESOURCE_NAME).requiresPermissions("samples:leishmania:public:*:view");
		return findNearbyLeishmaniaSamples(Point.builder().coordinates(LngLatAlt.builder().coordinates(longitude, latitude).build()).build(), 
	...
	/**
	 * Wraps a collection of {@link LeishmaniaSample}.
	 * @author Erik Torres <ertorser@upv.es>
	 */
	public static class Samples extends Paginable<LeishmaniaSample> {		

		@InjectLinks({
			@InjectLink(resource=LeishmaniaSampleResource.class, method="getSamples", bindings={
		...
		public static class SamplesBuilder {
			...
			public SamplesBuilder samples(final List<LeishmaniaSample> samples) {
