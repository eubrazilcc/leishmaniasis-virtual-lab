	...
	public void test() {
		...
		try {
			...
			for (final File file : files) {
				...
				for (final Taxon taxon : taxaSet.getTaxon()) {
					assertThat("Taxonomy XML taxon Id is not null", taxon.getTaxId(), notNullValue());
					assertThat("Taxonomy XML taxon Id value is not empty", trim(taxon.getTaxId().getvalue()), allOf(notNullValue(), not(equalTo(""))));
					assertThat("Taxonomy XML taxon scientific name is not empty", trim(taxon.getScientificName()), allOf(notNullValue(), not(equalTo(""))));
					assertThat("Taxonomy XML taxon division is not empty", trim(taxon.getDivision()), allOf(notNullValue(), not(equalTo(""))));
					assertThat("Taxonomy XML taxon rank is not empty", trim(taxon.getRank()), allOf(notNullValue(), not(equalTo(""))));
				}
			}
