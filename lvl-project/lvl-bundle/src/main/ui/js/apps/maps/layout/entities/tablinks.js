/**
 * RequireJS module that defines the entity: maps->tab-link.
 */

define([ 'app', 'backbone.picky' ], function(Lvl) {
	Lvl.module('MapsApp.Entities', function(Entities, Lvl, Backbone, Marionette, $, _) {
		...
		Entities.NavigationMaps = Backbone.Collection.extend({
		...
		var iniNavigationLinks = function() {
			Entities.navigationLinks = new Entities.NavigationMaps([ {
				id : 1,
				link : 'datasets',
				icon : 'fa-globe',
				text : 'Datasets'
		...
		Lvl.reqres.setHandler('maps:navigation:entities', function() {
