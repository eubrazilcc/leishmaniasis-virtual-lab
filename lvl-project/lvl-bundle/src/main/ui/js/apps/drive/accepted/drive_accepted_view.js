/**
 * RequireJS module that defines the view: drive->accepted.
 */

define([ 'app', 'marionette', 'tpl!apps/drive/accepted/tpls/drive_accepted', 'tpl!apps/drive/accepted/tpls/toolbar_browse', 'tpl!common/search/tpls/search_term',
		'tpl!common/search/tpls/add_search_term', 'tpl!common/search/tpls/save_search', 'pace', 'moment', 'backbone.oauth2', 'backgrid', 'backgrid-paginator',
		'backgrid-select-all', 'backgrid-filter' ], function(Lvl, Marionette, AcceptedTpl, ToolbarTpl, SearchTermTpl, AddSearchTermTpl, SaveSearchTpl, pace,
		moment) {
	Lvl.module('DriveApp.Accepted.View', function(View, Lvl, Backbone, Marionette, $, _) {
		'use strict';
		var columns = [		              
						{
							name : 'id',
							...
							cell : Backgrid.Cell.extend({
								render : function() {
									...
									if (formattedValue && typeof formattedValue === 'string') {
										this.$el.append('<a href="#" title="Open" class="text-muted" data-open="' + formattedValue
												+ '"><i class="fa fa-times fa-eye"></i></a>');
									}
									this.delegateEvents();
									return this;
								}
		...
		View.Content = Marionette.ItemView.extend({
			...
			initialize : function() {
				...
				// setup search
				Lvl.vent.on('search:form:submitted', this.searchAccepted);
				// setup menu
			...
			searchAccepted : function(search) {
			...
			startTour : function(e) {
				e.preventDefault();
				require([ 'apps/drive/accepted/tours/accepted_tour' ], function(tour) {
					tour();
				});
			},
			shortenUrl : function(e) {
				...
				var self = this;
				var target = $(e.target);
				var itemId = target.is('i') ? target.parent('a').get(0).getAttribute('data-shortened-url') : target.attr('data-shortened-url');
				$.ajax({
					type : 'GET',
					dataType : 'text',
					crossDomain : true,
			...
			removeLink : function(e) {
				...
				var self = this;
				var target = $(e.target);
				var itemId = target.is('i') ? target.parent('a').get(0).getAttribute('data-open') : target.attr('data-open');
				var item = this.collection.get(itemId);
				this.trigger('drive:view:obj_accepted', item);
			},			
			onDestroy : function() {
				pace.stop();
				this.stopListening();
	...
	return Lvl.DriveApp.Accepted.View;
});