/**
 * RequireJS module that defines the view: drive->granted.
 */

define([ 'app', 'marionette', 'tpl!apps/drive/granted/tpls/drive_granted', 'tpl!apps/drive/granted/tpls/toolbar_browse', 'tpl!common/search/tpls/search_term',
		...
		'backgrid-select-all', 'backgrid-filter' ], function(Lvl, Marionette, GrantedTpl, ToolbarTpl, SearchTermTpl, AddSearchTermTpl, SaveSearchTpl, pace,
		moment) {
	Lvl.module('DriveApp.Granted.View', function(View, Lvl, Backbone, Marionette, $, _) {
		'use strict';
		var columns = [		              
				{
					name : 'id',
		...
		View.Content = Marionette.ItemView.extend({
			...
			initialize : function() {
				...
				// setup search
				Lvl.vent.on('search:form:submitted', this.searchGranted);
				// setup menu
			...
			startTour : function(e) {
				e.preventDefault();
				require([ 'apps/drive/granted/tours/granted_tour' ], function(tour) {
					tour();
				});
			},
			...
			removeLink : function(e) {
				...
				require([ 'common/confirm' ], function(confirmDialog) {
					confirmDialog('Confirm deletion', 'This action will delete the selected public link. Are you sure?', function() {
	...
	return Lvl.DriveApp.Granted.View;
});