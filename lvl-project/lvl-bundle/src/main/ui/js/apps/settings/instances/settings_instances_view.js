...
define([ 'app', 'tpl!apps/settings/instances/tpls/settings_instances', 'tpl!apps/settings/instances/tpls/toolbar_browse', 'tpl!common/search/tpls/search_term',
		'tpl!common/search/tpls/add_search_term', 'tpl!common/search/tpls/save_search', 'entities/reference', 'pace', 'moment', 'common/country_names',
		'backbone.oauth2', 'backgrid', 'backgrid-paginator', 'backgrid-select-all', 'backgrid-filter' ], function(Lvl, InstanceTpl, ToolbarTpl, SearchTermTpl,
		AddSearchTermTpl, SaveSearchTpl, SequenceModel, pace, moment, mapCn) {
			...
			cell : Backgrid.Cell.extend({
				render : function() {
					...
					if (formattedValue && typeof formattedValue === 'number') {
						var color = (formattedValue + 300000) > new Date().getTime() ? 'text-success' : 'text-danger';
