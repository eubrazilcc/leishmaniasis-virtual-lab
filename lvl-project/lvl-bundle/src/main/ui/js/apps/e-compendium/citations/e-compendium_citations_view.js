/**
 * RequireJS module that defines the view: e-compendium->citations.
 */

define([ 'app', 'tpl!apps/e-compendium/citations/tpls/e-compendium_citations', 'tpl!apps/e-compendium/citations/tpls/toolbar_citations',
		'tpl!common/search/tpls/search_term', 'tpl!common/search/tpls/add_search_term', 'tpl!common/search/tpls/save_search', 'entities/reference', 'pace',
		'common/country_names', 'backbone.oauth2', 'backgrid', 'backgrid-paginator', 'backgrid-select-all', 'backgrid-filter' ], function(Lvl, CitationsTpl,
		ToolbarTpl, SearchTermTpl, AddSearchTermTpl, SaveSearchTpl, ReferenceEntity, pace, mapCn) {
	Lvl.module('ECompendiumApp.Citations.View', function(View, Lvl, Backbone, Marionette, $, _) {
		'use strict';
		var columns = [ {
			name : 'pubmedId',
			label : 'PMID',
		...
		View.Content = Marionette.ItemView.extend({
			...
			initialize : function() {
				...
				// setup search
				Lvl.vent.on('search:form:submitted', this.searchCitations);
				// setup menu
			...
			searchCitations : function(search) {
			...
			startTour : function(e) {
				e.preventDefault();
				require([ 'apps/e-compendium/citations/tours/e-compendium_tour' ], function(tour) {
					tour();
				});
			},
	...
	return Lvl.ECompendiumApp.Citations.View;
});