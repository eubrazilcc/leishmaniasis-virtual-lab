/**
 * RequireJS module that defines the view: collection->unsequenced.
 */

define([ 'app', 'tpl!apps/collection/unseq/tpls/collection_unseq', 'tpl!apps/collection/unseq/tpls/toolbar_unseq', 'tpl!common/search/tpls/search_term',
		...
		'common/country_names', 'backbone.oauth2', 'backgrid', 'backgrid-paginator', 'backgrid-select-all', 'backgrid-filter', 'common/ext/backgrid_ext' ], function(Lvl, UnseqTpl,
		ToolbarTpl, SearchTermTpl, AddSearchTermTpl, SaveSearchTpl, SequenceEntity, SavedSearchEntity, IdentifierEntity, pace, mapCn) {
	Lvl.module('CollectionApp.Unseq.View', function(View, Lvl, Backbone, Marionette, $, _) {
		'use strict';
		var columns = [
				{
					name : 'dataSource',
		...
		View.Content = Marionette.ItemView.extend({
			...
			handleDragStart : function(e) {
				...
				e.originalEvent.dataTransfer.setData('savable', JSON.stringify(new SavedSearchEntity.SavedSearch({
					type : 'collection;unseq;' + self.data_source,
					description : '',
			...
			startTour : function(e) {
				e.preventDefault();
				require([ 'apps/collection/unseq/tours/collection_tour' ], function(tour) {
					tour();
				});
			},
			...
			onShow : function() {
				...
				if (!$.isEmptyObject(params) && (params instanceof SavedSearchEntity.SavedSearch)
						&& (params.get('type') === 'collection;unseq;' + _self.data_source)) {
					var search = '';
	...
	return Lvl.CollectionApp.Unseq.View;
});